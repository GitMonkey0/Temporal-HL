#!/usr/bin/env python3
"""Learned gate for feasible-subset left repair.

This is an experiment memo, not paper text.

Use a richer but still lightweight feature set than the failed subtype-average
gate:

- other-hand motion
- interaction motion
- no-repair left/right state agreement
- no-repair left/right transition agreement

Train on the feasible `val` subset and test on the feasible `test` subset.
"""
...
def feature_vector(row):
    feats = [
        float(row["left_none_left_state_agreement"]),
        float(row["left_none_left_transition_agreement"]),
        float(row["left_none_right_state_agreement"]),
        float(row["left_none_right_transition_agreement"]),
    ]
...
def train_gate(train_rows):
    ...
    for row in train_rows:
        vals = {mode: float(row[f"left_{mode}_joint_score"]) for mode in LEFT_REPAIR_MODES}
        best_mode = max(vals, key=vals.get)
        xs.append(feature_vector(row))
        ys.append(LEFT_REPAIR_MODES.index(best_mode))
    x = np.asarray(xs, dtype=np.float32)
    y = np.asarray(ys, dtype=np.int32)
    counts = Counter(y.tolist())
    class_weight = {cls: len(y) / max(len(counts) * cnt, 1) for cls, cnt in counts.items()}
    sample_weight = np.asarray([class_weight[int(v)] for v in y], dtype=np.float32)
    ...
    return model, {
        ...
        "label_counts": {LEFT_REPAIR_MODES[k]: int(v) for k, v in counts.items()},
    }
...
def run_task(train_frames, test_frames, pair_bank, task_target: str):
    ...
    for row in test_rows:
        pred = LEFT_REPAIR_MODES[int(gate_model.predict(feature_vector(row).reshape(1, -1))[0])]
        for name, source in [
        ...
        vals = {mode: row[f"left_{mode}_joint_score"] for mode in LEFT_REPAIR_MODES}
        best_mode = max(vals, key=vals.get)
        for field in ("right_grouped_match", "left_preserve", "joint_score"):
            row[f"oracle_{field}"] = row[f"left_{best_mode}_{field}"]
        row["pred_mode"] = pred
        row["oracle_mode"] = best_mode
...
def main():
    ...
    payload = {
        ...
        "focus": {
            "goal": "richer learned gate for feasible left repair",
    ...
    out_json = GEN / f"interaction_realized_feasible_left_repair_model_gate{suffix}.json"
    out_md = SUM / f"interaction_realized_feasible_left_repair_model_gate{suffix}.md"
    out_json.write_text(json.dumps(payload, indent=2))

    lines = [
        "# Interaction Realized Feasible Left Repair Model Gate",
        "",
        "This is an experiment memo, not paper text.",
        "",
        "Richer learned gate over feasible-subset left repair.",
        "",
    ]
