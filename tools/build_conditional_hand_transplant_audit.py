#!/usr/bin/env python3
"""Conditional hand-transplant audit for hand-motion edits."""
...
def target_hand_distance(original_geom: dict[str, object], candidate_geom: dict[str, object], task_field: str) -> float:
    hand_name = target_hand_name(task_field)
    return hand_delta(original_geom[hand_name], candidate_geom[hand_name])
...
def pick_best_symbolic_donor(train_bank, original_attrs, original_geom, task_field, target_value):
    cands = matching_symbolic_candidates(train_bank, original_attrs, task_field, target_value)
    if not cands:
        return None
    cands.sort(key=lambda row: (target_hand_distance(original_geom, row["geom"], task_field), row["seq_name"], row["frame_idx"]))
    return cands[0]
...
def pick_best_proxy_donor(train_bank, source_name, cluster_decoded, proxy_attrs, original_geom, task_field, target_value):
        ...
        out.append(
            {
                "task": task,
                "num_frames": n,
                "symbolic_target_hand_delta": sum(row["symbolic_target_hand_delta"] for row in items) / n,
                "proxy_target_hand_delta": sum(row["proxy_target_hand_delta"] for row in items) / n,
...
def main():
            ...
            for task_field, task_target in TASKS:
                for start, end, value in contiguous_runs(frames, task_field):
                    ...
                    for idx in range(start, end):
                        ...
                        attrs = frame_attrs(frame)
                        geom = frame_geom(frame)
                        sym = pick_best_symbolic_donor(train_bank, attrs, geom, task_field, task_target)
                        if sym is None:
                            continue
                        proxy_attrs = pick_best_proxy_attrs(attrs, task_field, task_target, decoded)
                        if proxy_attrs is None:
                            continue
                        prox = pick_best_proxy_donor(train_bank, source_name, decoded, proxy_attrs, geom, task_field, task_target)
                        if prox is None:
                            continue
                        keep = preserved_fields(task_field)
                        ...
                        sym_delta = target_hand_distance(geom, sym["geom"], task_field)
                        prox_delta = target_hand_distance(geom, prox["geom"], task_field)
                        rows.append(
                            {
                                "task": f"{task_field}->{task_target}",
                                "seq_name": sequence["seq_name"],
                                "frame_idx": frame["frame_idx"],
                                "symbolic_target_hand_delta": sym_delta,
                                "proxy_target_hand_delta": prox_delta,
                                "symbolic_beats_proxy": int(sym_delta <= prox_delta),
    ...
    out_json = GEN / "conditional_hand_transplant_audit.json"
    out_md = SUM / "conditional_hand_transplant_audit.md"
    ...
    lines = [
        ...
        "",
        "This audit compares target-hand donor quality under a conditional hand-only transplant protocol.",
    ]
    for source in payload["sources"]:
        lines.extend([
            ...
            "",
            "| task | frames | symbolic target-hand delta | proxy target-hand delta | symbolic beats proxy | proxy preserved clean | proxy preserved mismatch | proxy semantic collateral |",
            "| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: |",
        ])
        for row in source["summary"]:
            lines.append(
                f"| {row['task']} | {row['num_frames']} | {fmt(row['symbolic_target_hand_delta'])} | {fmt(row['proxy_target_hand_delta'])} | "
                f"{fmt(row['symbolic_beats_proxy_rate'])} | {fmt(row['proxy_preserved_clean_rate'])} | {fmt(row['proxy_preserved_mismatch_fields'])} | {fmt(row['proxy_total_semantic_collateral'])} |"
            )
    out_md.write_text("\n".join(lines) + "\n")
