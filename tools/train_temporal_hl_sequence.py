#!/usr/bin/env python3
"""Sequence encoder over frame-level temporal HL features."""
...
from tools.train_refined_phase_pretrain import load_json, set_seed
...
def build_frame_vocabs(datasets: list[dict[str, object]]) -> dict[str, list[str]]:
    state_vocab = set()
    transition_vocab = set()
    hand_motion_vocab = set()
            ...
            for frame in sequence["frames"]:
                for hand_name in ("right", "left"):
                    ...
                    for token in hand_record.get("token_labels", []):
                        state_vocab.add(f"{hand_name}:state::{token}")
                    for token in hand_record.get("transition_labels", []):
                        transition_vocab.add(f"{hand_name}:trans::{token}")
                    hand_motion_vocab.add(
                        f"{hand_name}:motion::{hand_record.get('hand_motion', 'unknown')}"
                    )
                interaction_vocab.add(
                    f"interaction::{frame.get('interaction_motion', 'unknown')}"
                )
    return {
        "state": sorted(state_vocab),
        "transition": sorted(transition_vocab),
        "hand_motion": sorted(hand_motion_vocab),
        "interaction": sorted(interaction_vocab),
    }


def frame_to_temporal_vector(
    frame: dict[str, object],
    vocabs: dict[str, list[str]],
    include_duration: bool = False,
) -> np.ndarray:
    state_index = {key: idx for idx, key in enumerate(vocabs["state"])}
    transition_index = {key: idx for idx, key in enumerate(vocabs["transition"])}
    motion_index = {key: idx for idx, key in enumerate(vocabs["hand_motion"])}
    inter_index = {key: idx for idx, key in enumerate(vocabs["interaction"])}
    ...
    offset_state = 0
    offset_trans = offset_state + len(state_index)
    offset_motion = offset_trans + len(transition_index)
    offset_inter = offset_motion + len(motion_index)
    offset_extra = offset_inter + len(inter_index)

    for hand_name in ("right", "left"):
        hand_record = frame.get(hand_name)
        if hand_record is None:
            continue
        token_counts = Counter(hand_record.get("token_labels", []))
        for token, count in token_counts.items():
            key = f"{hand_name}:state::{token}"
            vec[offset_state + state_index[key]] = count / 20.0
        trans_counts = Counter(hand_record.get("transition_labels", []))
        for token, count in trans_counts.items():
            key = f"{hand_name}:trans::{token}"
            vec[offset_trans + transition_index[key]] = count / 20.0
        motion_key = f"{hand_name}:motion::{hand_record.get('hand_motion', 'unknown')}"
        vec[offset_motion + motion_index[motion_key]] = 1.0

    inter_key = f"interaction::{frame.get('interaction_motion', 'unknown')}"
    vec[offset_inter + inter_index[inter_key]] = 1.0

    right = frame.get("right")
    left = frame.get("left")
    all_transition_labels = []
    for hand_record in (right, left):
        if hand_record is not None:
            all_transition_labels.extend(hand_record.get("transition_labels", []))
    non_stay = sum(token not in ("start", "stay") for token in all_transition_labels)
    major = sum(token == "major_shift" for token in all_transition_labels)
    total = max(len(all_transition_labels), 1)
    vec[offset_extra + 0] = non_stay / total
    vec[offset_extra + 1] = major / total
    vec[offset_extra + 2] = 1.0 if frame.get("interaction_motion") == "approach" else 0.0
    vec[offset_extra + 3] = 1.0 if frame.get("interaction_motion") == "separate" else 0.0

    if include_duration:
        duration_keys = [
            ("right", "state_persistence_label"),
            ("right", "activity_persistence_label"),
            ("left", "state_persistence_label"),
            ("left", "activity_persistence_label"),
        ]
        duration_map = {"start": 0.25, "instant": 0.5, "short": 0.75, "medium": 1.0, "long": 1.25}
        cursor = offset_extra + 4
        for hand_name, field in duration_keys:
            hand_record = frame.get(hand_name)
            value = "start" if hand_record is None else hand_record.get(field, "start")
            vec[cursor] = duration_map.get(value, 0.0)
            cursor += 1
        vec[cursor] = duration_map.get(frame.get("interaction_persistence_label", "start"), 0.0)
        cursor += 1
        vec[cursor] = duration_map.get(frame.get("interaction_activity_persistence_label", "start"), 0.0)
        cursor += 1
        vec[cursor] = duration_map.get(frame.get("interaction_segment_duration_label", "start"), 0.0)
        cursor += 1
        vec[cursor] = duration_map.get(frame.get("interaction_activity_segment_duration_label", "start"), 0.0)
    return vec


def make_window_temporal_sequence_samples(
    ...
    for seq_idx, sequence in enumerate(dataset["sequences"]):
        ...
        for window_idx, (start, end) in enumerate(window_ranges):
            ...
            frame_features = [
                frame_to_temporal_vector(frame, vocabs, include_duration=include_duration)
                for frame in window
            ]
...
class TemporalHLSequenceEncoder(nn.Module):
...
def train_model(
    model: TemporalHLSequenceEncoder,
    train_x: np.ndarray,
    train_y: np.ndarray,
...
def run_once(
    ...
    scratch = TemporalHLSequenceEncoder(frame_dim, hidden_dim, len(ft_labels)).to(device)
    ...
    pretrained = TemporalHLSequenceEncoder(frame_dim, hidden_dim, len(pre_labels)).to(device)
    ...
    finetune = TemporalHLSequenceEncoder(frame_dim, hidden_dim, len(ft_labels)).to(device)
...
def main():
    ...
    parser.add_argument("--output", type=Path, default=Path("experiments/generated/temporal_hl_sequence.json"))
    ...
    vocabs = build_frame_vocabs([pre_data, ft_data, test_data])
    ...
    pre_samples = make_window_temporal_sequence_samples(
        pre_data,
    ...
    ft_samples = make_window_temporal_sequence_samples(
        ft_data,
        allowed,
        vocabs,
        args.window_size,
    ...
    test_samples = make_window_temporal_sequence_samples(
        test_data,
        allowed,
        vocabs,
        args.window_size,
