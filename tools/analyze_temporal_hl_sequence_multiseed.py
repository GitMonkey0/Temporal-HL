#!/usr/bin/env python3
"""Aggregate multi-seed sequence errors for temporal-HL sequence experiments."""
...
from tools.train_temporal_hl_sequence import (
    TemporalHLSequenceEncoder,
    build_frame_vocabs,
    frame_to_temporal_vector,
    make_window_temporal_sequence_samples,
    predict_probs,
    train_model,
)
...
def main():
    ...
    parser.add_argument("--output", type=Path, default=Path("experiments/generated/temporal_hl_sequence_analysis.json"))
    ...
    ft_samples = make_window_temporal_sequence_samples(
        ft_data, allowed, vocabs, args.window_size, args.stride,
        include_duration=args.include_duration, window_span_units=ft_span, window_step_units=ft_step
    )
    test_samples = make_window_temporal_sequence_samples(
        test_data, allowed, vocabs, args.window_size, args.stride,
        include_duration=args.include_duration, window_span_units=test_span, window_step_units=test_step
    )
    ...
    payload = {
        ...
        "scratch_error_counts": [
            ...
            for (t, p), c in scratch_error_counter.most_common()
        ],
        "pretrained_error_counts": [
            {"target": t, "prediction": p, "count": c}
            for (t, p), c in pretrained_error_counter.most_common()
        ],
    }
    ...
    print(
        "scratch_seq_mean",
        float(np.mean([run["scratch_sequence_accuracy"] for run in runs])),
        "pretrained_seq_mean",
        float(np.mean([run["pretrained_sequence_accuracy"] for run in runs])),
    )
