#!/usr/bin/env python3
"""Aggregate multi-seed sequence errors for symbolic pretraining experiments."""
...
from tools.analyze_symbolic_pretrain import train_eval_details
...
from tools.train_symbolic_classifier import canonical_label, subsample_by_fraction
...
def main():
    ...
    parser.add_argument("--output", type=Path, default=Path("experiments/generated/symbolic_pretrain_multiseed_analysis.json"))
    ...
    for seq_name in all_seq:
        scratch_correct = sum(item["correct"] for item in scratch_per_seq[seq_name])
        pretrained_correct = sum(item["correct"] for item in pretrained_per_seq[seq_name])
        seq_summary[seq_name] = {
            "scratch_correct_runs": scratch_correct,
            "pretrained_correct_runs": pretrained_correct,
